...
struct scheduler::task_cleanup
{
  ~task_cleanup()
  {
    ...
    {
      ASIO_NAMESPACE::detail::increment(
...
scheduler::scheduler(ASIO_NAMESPACE::execution_context& ctx,
    int concurrency_hint, bool own_thread, get_task_func_type get_task)
  : ASIO_NAMESPACE::detail::execution_context_service_base<scheduler>(ctx),
    ...
    thread_(0)
{
  ...
  {
    ++outstanding_work_;
    ASIO_NAMESPACE::detail::signal_blocker sb;
    thread_ = new ASIO_NAMESPACE::detail::thread(thread_function(this));
  }
}
...
std::size_t scheduler::run(ASIO_NAMESPACE::error_code& ec)
{
...
std::size_t scheduler::run_one(ASIO_NAMESPACE::error_code& ec)
{
  ec = ASIO_NAMESPACE::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::wait_one(long usec, ASIO_NAMESPACE::error_code& ec)
{
  ec = ASIO_NAMESPACE::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::poll(ASIO_NAMESPACE::error_code& ec)
{
  ec = ASIO_NAMESPACE::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::poll_one(ASIO_NAMESPACE::error_code& ec)
{
  ec = ASIO_NAMESPACE::error_code();
  if (outstanding_work_ == 0)
  {
...
scheduler_task* scheduler::get_default_task(ASIO_NAMESPACE::execution_context& ctx)
{
#if defined(ASIO_HAS_IO_URING_AS_DEFAULT)
