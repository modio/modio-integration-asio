    ...
    winrt_ssocket_service_base::base_implementation_type& other_impl)
{
  ...
  {
    // Insert implementation into linked list of all implementations.
    ASIO_NAMESPACE::detail::mutex::scoped_lock lock(other_service.mutex_);
...
ASIO_NAMESPACE::error_code winrt_ssocket_service_base::close(
    winrt_ssocket_service_base::base_implementation_type& impl,
    ...
    void* addr, std::size_t addr_len, ASIO_NAMESPACE::error_code& ec) const
{
  ...
  {
    ec = ASIO_NAMESPACE::error_code(e->HResult,
        ASIO_NAMESPACE::system_category());
    return addr_len;
  }
}

ASIO_NAMESPACE::error_code winrt_ssocket_service_base::do_set_option(
    winrt_ssocket_service_base::base_implementation_type& impl,
...
ASIO_NAMESPACE::error_code winrt_ssocket_service_base::do_connect(
    winrt_ssocket_service_base::base_implementation_type& impl,
    ...
    const void* addr, winrt_async_op<void>* op, bool is_continuation)
{
  ...
  case ASIO_OS_DEF(AF_INET6):
    ...
    break;
  default:
    op->ec_ = ASIO_NAMESPACE::error::address_family_not_supported;
    break;
  }
  ...
  catch (Platform::Exception^ e)
  {
    op->ec_ = ASIO_NAMESPACE::error_code(
        e->HResult, ASIO_NAMESPACE::system_category());
    scheduler_.post_immediate_completion(op, is_continuation);
  }
}
    ...
    socket_base::message_flags flags, ASIO_NAMESPACE::error_code& ec)
{
  ...
  try
  {
    buffer_sequence_adapter<ASIO_NAMESPACE::const_buffer,
      ASIO_NAMESPACE::const_buffer> bufs(ASIO_NAMESPACE::buffer(data));
      ...
      winrt_async_op<unsigned int>* op, bool is_continuation)
{
  if (flags)
  {
    op->ec_ = ASIO_NAMESPACE::error::operation_not_supported;
    scheduler_.post_immediate_completion(op, is_continuation);
    return;
  }
  ...
  {
    op->ec_ = ASIO_NAMESPACE::error_code(e->HResult,
        ASIO_NAMESPACE::system_category());
    scheduler_.post_immediate_completion(op, is_continuation);
  }
}
    ...
    socket_base::message_flags flags, ASIO_NAMESPACE::error_code& ec)
{
  ...
  try
  {
    buffer_sequence_adapter<ASIO_NAMESPACE::mutable_buffer,
        ASIO_NAMESPACE::mutable_buffer> bufs(ASIO_NAMESPACE::buffer(data));
