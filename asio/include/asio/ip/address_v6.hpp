...
/**
 * The ASIO_NAMESPACE::ip::address_v6 class provides the ability to use and
...
#else
  typedef ASIO_NAMESPACE::detail::array<unsigned char, 16> bytes_type;
#endif
...
private:
  ...
  ASIO_NAMESPACE::detail::in6_addr_type addr_;
 ...
 *
 * @relates ASIO_NAMESPACE::ip::address_v6
 */
...
template <>
struct hash<ASIO_NAMESPACE::ip::address_v6>
{
  std::size_t operator()(const ASIO_NAMESPACE::ip::address_v6& addr)
    const ASIO_NOEXCEPT
  {
    const ASIO_NAMESPACE::ip::address_v6::bytes_type bytes = addr.to_bytes();
