...
namespace detail
{
  struct default_connect_condition
  {
    ...
    bool operator()(const ASIO_NAMESPACE::error_code&, const Endpoint&)
    {
      return true;
    }
  };
  ...
  inline typename Protocol::endpoint deref_connect_result(
      Iterator iter, ASIO_NAMESPACE::error_code& ec)
  {
    ...
    static const bool value =
      sizeof(asio_connect_condition_check(
        (declval<legacy_connect_condition_helper<T, Iterator> >())(
          declval<const ASIO_NAMESPACE::error_code>(),
  ...
  inline Iterator call_connect_condition(ConnectCondition& connect_condition,
      const ASIO_NAMESPACE::error_code& ec, Iterator next, Iterator end,
    ...
    ASIO_NAMESPACE::error_code& ec)
{
  ...
  if (!ec)
    ec = ASIO_NAMESPACE::error::not_found;
...
namespace detail
{
  ...
  protected:
    ...
    void check_condition(const ASIO_NAMESPACE::error_code& ec,
  ...
  protected:
    ...
    template <typename Iterator>
    void check_condition(const ASIO_NAMESPACE::error_code&, Iterator&, Iterator&)
    {
    }
  };
  ...
  public:
    ...
    void operator()(ASIO_NAMESPACE::error_code ec, int start = 0)
    {
  ...
  //private:
    template <typename Iterator>
    void process(ASIO_NAMESPACE::error_code ec,
      ...
      switch (start_ = start)
      {
        ...
        for (;;)
        {
          ...
          if (start)
          {
            ec = ASIO_NAMESPACE::error::not_found;
            ASIO_HANDLER_LOCATION((__FILE__, __LINE__, "async_connect"));
            ASIO_NAMESPACE::post(socket_.get_executor(),
                detail::bind_handler(
        ...
        const ConnectCondition& connect_condition) const
    {
      ...
      range_connect_op<Protocol, Executor, EndpointSequence, ConnectCondition,
        typename decay<RangeConnectHandler>::type>(socket_, endpoints,
          connect_condition, handler2.value)(ASIO_NAMESPACE::error_code(), 1);
    }
