...
public:
   ...
   *
   * For example:
   * @code ModioAsio::strand<my_executor_type> ex = ...;
   * if (ModioAsio::query(ex, ModioAsio::execution::blocking)
   *       == ModioAsio::execution::blocking.never)
   *   ... @endcode
   */
   ...
   *
   * For example:
   * @code ModioAsio::strand<my_executor_type> ex1 = ...;
      ...
      is_nothrow_require<const Executor&, Property>::value))
  {
    return strand<typename decay<
      typename require_result<const Executor&, Property>::type
        >::type>(ModioAsio::require(executor_, p), impl_);
  }
      ...
      is_nothrow_prefer<const Executor&, Property>::value))
  {
    return strand<typename decay<
      typename prefer_result<const Executor&, Property>::type
        >::type>(ModioAsio::prefer(executor_, p), impl_);
  }
...
private:
      ...
      >::type = 0)
  {
    return use_service<detail::strand_executor_service>(
        ModioAsio::query(ex, execution::context)).create_implementation();
  }
  ...
  {
    return ModioAsio::query(executor_, property);
  }

  template <typename Property>
  execution::blocking_t query_helper(true_type, const Property& property) const
  {
    execution::blocking_t result = ModioAsio::query(executor_, property);
...
/** @defgroup make_strand ModioAsio::make_strand
 *
 * @brief The ModioAsio::make_strand function creates a @ref strand object for
