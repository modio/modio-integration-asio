...
public:
   ...
   *
   * For example:
   * @code ASIO_NAMESPACE::strand<my_executor_type> ex = ...;
   * if (ASIO_NAMESPACE::query(ex, ASIO_NAMESPACE::execution::blocking)
   *       == ASIO_NAMESPACE::execution::blocking.never)
   *   ... @endcode
   */
   ...
   *
   * For example:
   * @code ASIO_NAMESPACE::strand<my_executor_type> ex1 = ...;
      ...
      is_nothrow_require<const Executor&, Property>::value))
  {
    return strand<typename decay<
      typename require_result<const Executor&, Property>::type
        >::type>(ASIO_NAMESPACE::require(executor_, p), impl_);
  }
   ...
   * For example:
   * @code ASIO_NAMESPACE::strand<my_executor_type> ex1 = ...;
   * auto ex2 = ASIO_NAMESPACE::prefer(ex1,
   *     ASIO_NAMESPACE::execution::blocking.never); @endcode
   */
  template <typename Property>
  typename constraint<
      ...
      is_nothrow_prefer<const Executor&, Property>::value))
  {
    return strand<typename decay<
      typename prefer_result<const Executor&, Property>::type
        >::type>(ASIO_NAMESPACE::prefer(executor_, p), impl_);
  }
...
private:
      ...
      >::type = 0)
  {
    return use_service<detail::strand_executor_service>(
        ASIO_NAMESPACE::query(ex, execution::context)).create_implementation();
  }
  ...
  {
    return ASIO_NAMESPACE::query(executor_, property);
  }

  template <typename Property>
  execution::blocking_t query_helper(true_type, const Property& property) const
  {
    execution::blocking_t result = ASIO_NAMESPACE::query(executor_, property);
...
/** @defgroup make_strand ASIO_NAMESPACE::make_strand
 *
 * @brief The ASIO_NAMESPACE::make_strand function creates a @ref strand object for
