...
class logger_service
  : public ASIO_NAMESPACE::execution_context::service
{
public:
  ...
  logger_service(ASIO_NAMESPACE::execution_context& context)
    : ASIO_NAMESPACE::execution_context::service(context),
      work_io_context_(),
      work_(ASIO_NAMESPACE::make_work_guard(work_io_context_)),
      work_thread_(new ASIO_NAMESPACE::thread(
            boost::bind(&ASIO_NAMESPACE::io_context::run, &work_io_context_)))
  {
  }
  ...
  void use_file(impl_type& /*impl*/, const std::string& file)
  {
    ...
    ASIO_NAMESPACE::post(work_io_context_, boost::bind(
...
private:
  ...
  ASIO_NAMESPACE::io_context work_io_context_;
  ...
  /// immediately.
  ASIO_NAMESPACE::executor_work_guard<
      ASIO_NAMESPACE::io_context::executor_type> work_;

  /// Thread used for running the work io_context's run loop.
  boost::scoped_ptr<ASIO_NAMESPACE::thread> work_thread_;
